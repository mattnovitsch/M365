...
if ($CollectDeviceInventory) {
    ...
    Confirm-SecureBootUEFI -ErrorAction SilentlyContinue -ErrorVariable ProcessError;
    If ($ProcessError) {
    ...
    $DeviceGuard = (Get-CimInstance –ClassName Win32_DeviceGuard –Namespace root\Microsoft\Windows\DeviceGuard | Select VirtualizationBasedSecurityStatus)
    
    If ($DeviceGuard.VirtualizationBasedSecurityStatus -contains 0)
    {
        $DeviceGuardStatus = 'Disabled'
    }
    Else
    {
        $DeviceGuardStatus = 'Enabled'
    }
    
        $DeviceGuardStatus

    $SHA256 = Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\IntegrityServices\' | select TPMDigestAlgID
    
    If ($SHA256.TPMDigestAlgID -contains 11)
    {
        $SHA256Status = 'Enabled'
    }
    Else
    {
    ...
    $Inventory | Add-Member -MemberType NoteProperty -Name "BitlockerVolumeStatus" -Value "$ConversionStatus" -Force
    ...
    $Inventory | Add-Member -MemberType NoteProperty -Name "CredentialGuard" -Value $CredentialGuardStatus -Force
    $Inventory | Add-Member -MemberType NoteProperty -Name "DeviceGuard" -Value $DeviceGuardStatus -Force
    $Inventory | Add-Member -MemberType NoteProperty -Name "SHA256" -Value $SHA256Status -Force
